...
const PendingOrdersTable: FC<PendingOrderTableProps> = ({orders, count = 0, limit = 10, page = 1, loading = true, handleSearch, refresh}) => {
     ...
     {!loading && <Table>
        ...
        <TableBody>
          {orderBy((orders || []), ["createdAt"], ["desc"])?.map((order) => (
            <PendingOrdersRow key={order?.["_id"]} order={order} />
          ))}
        </TableBody>
      </Table>}
