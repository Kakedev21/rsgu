...
export const useCartState = create<UseCartStateProps>((set) => ({
  ...
  setSelected: (cart: CartProps | null) =>
    set((state) => ({ ...state, selected: cart })),
  setUpdateCart: (value: boolean) =>
    set((state) => ({ ...state, updateCart: value }))
}));
...
const useCart = ({
  ...
  const changeCartQuantity = async (
    ...
    setLoading(true);
    const result = await axios.put('/api/bff/cart', { cart_id, action });
    setLoading(false);
    return result.data;
  };
